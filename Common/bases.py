...
class Serialisable(metaclass=SerialisableMeta):
    ...
    def decompose(self, target: type[T], /) -> T:
        ...
        if not issubclass(target, Serialisable):
            raise TypeError(f"{target.__name__} is not a subclass of Serialisable.")

        try:
            return target(self.json())

        except Exception as error:
            raise ValueError(
                f"{cls.__name__} cannot decompose into {target.__name__}: {error}"
            ) from error
